    ...
    if config.AUG.MIXUP == 0 and config.MODEL.LABEL_SMOOTHING > 0.0:
        if config.HIERARCHY.VARIANT == "multitask":
            raise NotImplementedError(
                "We don't support hierarchical multitask loss with label smoothing and no mixup."
            )
...
def build_loss_criterion_hot_ice(config, hierarchy, classes, weights):
    if config.AUG.MIXUP == 0 and config.MODEL.LABEL_SMOOTHING > 0.0:
        ...
        criterion = hierarchical.HierarchicalCrossEntropyLoss_hot_ice(
...
def build_loss_criterion_red_chilli(config, hierarchy, classes, weights):
    if config.AUG.MIXUP == 0 and config.MODEL.LABEL_SMOOTHING > 0.0:
        ...
        criterion = hierarchical.HierarchicalCrossEntropyLoss_red_chilli(
            hierarchy, classes, weights, coeffs=config.TRAIN.HIERARCHICAL_COEFFS
        ).to(torch.cuda.current_device())
    else:
        criterion = torch.nn.CrossEntropyLoss()
